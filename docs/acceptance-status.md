...
- 当前 `main` 生产发布包来源为 `7dbd487994187cf9490835b87c1983e48052217a`，已在腾讯云 `/tmp/way-memory-release-7dbd487` 暂存；`bun run release:build`、`bun run smoke:release` 和远端 9 个 payload 哈希校验均通过。远端尚未执行 `install-release.sh`，生产环境文件、DNS 和证书条件仍未满足，因此没有执行生产重启。
