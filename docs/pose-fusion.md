...
Vertical fusion uses a complementary filter: inertial Z motion is retained at high frequency, while fresh barometer altitude applies a small correction during movement and a stronger convergence while stationary. A short phone lift or drop therefore remains visible even before pressure has changed enough to estimate the new height; pressure is still used for longer elevator and stairs evidence.

Translation and rotation are intentionally separated: a phone can rotate in a blind user's hand while its center remains in place. Gyroscope activity is retained in the raw sensor stream, but zero translational acceleration and zero vertical pressure motion keep the fused pose stationary instead of turning angular motion into walking or an artificial route segment. On devices without a `linear-acceleration` sensor, the accelerometer-minus-gravity fallback is temporarily withheld during fast rotation because its lagging gravity estimate can otherwise turn a hand rotation into a false translation; step-PDR, ARCore, GNSS, and the raw streams remain available. The regression suite includes a pure-rotation case that asserts all three position axes remain at the origin.

GNSS fixes are also used to estimate a bounded velocity from successive monotonic fixes. A fix is treated as fresh for five seconds; after that, pose accuracy grows with the age of the last fix and the source flags change to `gnss-stale`. A horizontal GNSS jump that exceeds the bounded walking-speed and uncertainty gate is retained in raw evidence but does not move the fused pose; the pose exposes `gnss-rejected`, and a later good fix is compared with the last accepted reference rather than inheriting the outlier. The same freshness boundary is applied to barometer and visual evidence, so a previously available sensor cannot silently keep a high-confidence label after its stream has stopped. If GNSS altitude appears after a fix that had no altitude, the engine bridges it through the current relative height instead of resetting the z-axis to an unrelated zero.
...
ARCore uses a session-local metric frame. The adapter converts `+X right, +Y up, -Z forward` into the display frame `X right, Y forward, Z up`. It waits for at least `0.35m` visual displacement and `0.10m` inertial displacement before estimating horizontal alignment, so short phone movements can enter the unified route without using a single noisy frame. Samples before alignment remain diagnostic and are not promoted into the unified route. Promoted poses carry `visual` and `visual-aligned` source flags. ARCore does not provide a per-frame absolute covariance through this adapter, so the client uses a conservative quality prior and widens fused uncertainty when visual confidence is lower; it must not be interpreted as centimeter-level accuracy.

The fusion engine independently rejects every visual sample whose tracking state is not `tracking`, even if an adapter or replay accidentally supplies it. A paused/lost frame therefore cannot move the unified route; it also arms an internal reset so the next valid frame is re-anchored even if an adapter accidentally omits `trackingReset`.
