...
The fusion engine independently rejects every visual sample whose tracking state is not `tracking`, even if an adapter or replay accidentally supplies it. A paused/lost frame therefore cannot move the unified route; the next valid frame must re-anchor through the explicit `trackingReset` boundary.
