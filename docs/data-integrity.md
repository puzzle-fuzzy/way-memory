...
- The Android uploader caps its in-memory offline queue at 4,096 samples. It drops the oldest samples after that hard limit and surfaces the drop count in the capture UI. A short WebSocket interruption resumes against the same active session within a two-minute server grace window; an explicit stop cancels that grace window.
