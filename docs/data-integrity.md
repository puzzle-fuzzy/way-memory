...
- `sensorInventory` is the bounded capability snapshot sent at session start. It records the Android sensor string type, name, vendor metadata, timing/power hints, the client-side `transportMaxHz` diagnostic budget, and whether listener registration succeeded; `sensorStats` remains the separate observed-sample count.
